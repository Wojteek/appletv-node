...
const ed25519_supercop_1 = require("ed25519-supercop");
...
class Verifier {
    ...
    verify() {
        ...
        return that.device
            ...
            if (!ed25519_supercop_1.verify(signature, deviceInfo, that.device.credentials.publicKey)) {
                throw new Error('Signature verification failed');
            }
            let material = Buffer.concat([verifyPublic, Buffer.from(that.device.credentials.pairingId), sessionPublicKey]);
            const { secretKey, publicKey } = ed25519_supercop_1.createKeyPair(that.device.credentials.encryptionKey);
            const signed = ed25519_supercop_1.sign(material, publicKey, secretKey);
